...
class IngredientMatcher:
    ...
    def match(self, input_name: str) -> IngredientEntry:
        ...
        # 1. Exact Match
        exact: IngredientEntry | None = self.repo.get_ingredient_by_name(cleaned_input)
        if exact is not None:
            return exact

        # 2. Fuzzy Match
        all_names: List[str] = self.repo.get_all_ingredient_names()
        ...
        ingredient_entries: List[IngredientEntry | None] = [self.repo.get_ingredient_by_name(name) for name in matches]
